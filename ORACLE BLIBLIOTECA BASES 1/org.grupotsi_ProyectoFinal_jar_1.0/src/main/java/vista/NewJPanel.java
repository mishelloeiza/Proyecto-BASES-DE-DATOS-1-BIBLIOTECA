...
public class NewJPanel extends javax.swing.JPanel {
    ...
    private void buscarPermisosPorId(int id) {
        try (Connection cn = DriverManager.getConnection(JDBC_URL, USUARIO, CLAVE);
    ...
    // CONVERTIR RESULTSET A MODELO DE TABLA
    ...
    private void initComponents() {
        ...
        txtPermisoMantenimiento.setText("GESTIONES");

        txtPermisoProcesos.setText("MANTENIMIENTOS");
    ...
    // Acción del botón para cargar datos y mostrar en la tabla TABLAB
    DefaultTableModel model = cargarPermisosUsuarioDesdeBD();
    
    if (model != null) {
        TABLAB.setModel(model);
    } else {
        JOptionPane.showMessageDialog(this, "No se pudo cargar la tabla de permisos.");
    }
    ...
    try {
        ...
        try (Connection cn = DriverManager.getConnection(JDBC_URL, usuario, password);
             PreparedStatement pst = cn.prepareStatement(
                 "UPDATE permisos_usuario SET puede_mantenimiento=?, puede_procesos=?, puede_eliminar=?, puede_registrar=?, puede_modificar=?, "
            ...
            if (filasAfectadas > 0) {
                ...
                JOptionPane.showMessageDialog(this, "⚠️ No se encontró el usuario con ese ID.");
            }
        }
    } catch (Exception e) {
    ...
    try {
        ...
        try (Connection cn = DriverManager.getConnection(JDBC_URL, usuario, password);
             PreparedStatement pst = cn.prepareStatement(
                 "INSERT INTO permisos_usuario (id_usuario, puede_mantenimiento, puede_procesos, puede_eliminar, puede_registrar, puede_modificar, "
            ...
            if (filasAfectadas > 0) {
                ...
                JOptionPane.showMessageDialog(this, "⚠️ No se pudo agregar los permisos.");
            }
        }
    } catch (Exception e) {
    ...
    try {
        ...
        if (idText.isEmpty()) {
            JOptionPane.showMessageDialog(this, "⚠️ Ingrese un ID antes de buscar.");
            return;
        }

        int id = Integer.parseInt(idText);
        buscarPermisosPorId(id); // Este método ya está adaptado para Oracle XE
    ...
    // Desmarca los permisos de aplicaciones APL103 - APL112
    txtAPL103.setSelected(false);
    txtAPL104.setSelected(false);
    txtAPL105.setSelected(false);
    txtAPL106.setSelected(false);
